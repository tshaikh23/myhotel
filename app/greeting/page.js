...
const Greetings = () => {
  ...
  const createGreet = async () => {
    ...
    if (!trimmedGreet && !trimmedMessage) {
      setIsTimerPopupOpen(true); // Show popup if fields are empty
      setTimeout(() => {
        setIsTimerPopupOpen(false); // Close popup after 3 seconds
      }, 3000);
      return;
    }
    ...
    try {
      ...
      setCreateGreetData({ greet: '', message: '' });
      setGreetings([...greetings, response.data]); // Add new greet to the state
      setDataAdded(true); // Set dataAdded flag to true
    } catch (error) {
      if (error.response && error.response.status === 400) {
        // Handle the "Greet already exists" error from the backend
        setIsExitModalOpen('A greet already exists'); // Show specific error message
        setIsExitModalOpen(true); // Open the timer popup
        setTimeout(() => {
          setIsExitModalOpen(false); // Close the popup after 3 seconds
        }, 3000);
      } else {
        console.error('Error creating greet:', error);
      }
...
{isExitModalOpen && (
          <div className="fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center z-50">
            <div className="bg-white p-6 rounded-md shadow-md">
              <h2 className="text-base font-semibold mb-4 text-red-600">Greet Already Exit...</h2>
